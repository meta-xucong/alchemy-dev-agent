...
class CodexWorkerAdapter:
    ...
    def build_prompt(self, worker_input: CodexWorkerInput) -> str:
        ...
        return (
            ...
            "It must provide snapshot(), step(dt), advanceToVictory(), restart(), and runTraversalProbe(). snapshot() "
            "should return numeric player_x, numeric player_y, state, and won fields. runTraversalProbe() must execute "
            "the game's real fixed-step physics against level data and return {passed, scenarios}; every required "
            "high-obstacle, step, or gap scenario must include an id, kind, passed boolean, and measured evidence. At "
            "least one scenario must prove vertical clearance for the tallest required obstacle. A synthetic victory "
            "teleport does not substitute for this traversal proof. These hooks are for automated acceptance and must "
            "not require external assets or network calls.\n\n"
