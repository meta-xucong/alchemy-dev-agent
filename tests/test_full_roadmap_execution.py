...
class FullRoadmapExecutionTests(unittest.TestCase):
    ...
    def test_final_verification_resume_records_frontend_admin_account_timeout_focus(self) -> None:
        root = temp_root()
        doc = root / "roadmap.md"
        write_v3_docs(doc)
        output_dir = root / "final_verification"
        ...
        write_json(
            output_dir / "final_verification_worker_report.json",
            {
                ...
                "attempts": [
                    {
                        "attempt": 14,
                        "output_dir": str(output_dir / "run_attempt_014"),
                        "status": "blocked",
                    }
                ],
                "result": {
                                    ...
                                    "evidence": [
                                        {
                                            ...
                                            "result": {
                                                ...
                                                "worker_lifecycle": {
                                                    "status": "timed_out",
                                                    "timeout_seconds": 900,
                                                    "timed_out_at": "2026-06-28T22:31:14+00:00",
                                                },
                                            },
                                        }
                                    ],
        ...
        repair_docs = [Path(item) for item in captured[0]["documents"] if "final_verification_repair_resume" in str(item)]
        self.assertEqual(report["status"], "passed")
        self.assertEqual(repair_docs[-1].name, "final_verification_repair_resume_010.md")
        text = repair_docs[-1].read_text(encoding="utf-8")
        self.assertIn("Repair attempt: run_attempt_014", text)
        self.assertIn("Primary failed task IDs: T011", text)
        self.assertIn("Must continue focused task T011: Repair final frontend admin account identity components.", text)
        completed_line = next(line for line in text.splitlines() if "Completed tasks to preserve:" in line)
        for task_id in ("T001", "T002", "T003", "T004", "T005", "T006", "T007", "T008", "T009", "T010"):
            self.assertIn(task_id, completed_line)
        self.assertNotIn("T011", completed_line)
    ...
    def test_final_verification_resume_records_frontend_admin_user_account_timeout_focus(self) -> None:
        root = temp_root()
        doc = root / "roadmap.md"
        write_v3_docs(doc)
        output_dir = root / "final_verification"
        ...
        write_json(
            output_dir / "final_verification_worker_report.json",
            {
                ...
                "attempts": [
                    {
                        "attempt": 16,
                        "output_dir": str(output_dir / "run_attempt_016"),
                        "status": "blocked",
                    }
                ],
                "result": {
                    ...
                    "evidence": ["FINAL_AUDIT_STATUS=FAIL: source-boundary repair attempt needs continuation."],
                    "runtime_state": {
                        "completed_tasks": ["T012", "T013", "T014", "T015"],
                        "failed_tasks": ["T016"],
                        "blockers": [
                            {
                                "id": "B-T016-1",
                                "type": "technical_limit",
                                "description": "T016 exceeded the Codex worker timeout. Stop instead of launching a same-scope debug task.",
                                "task_ids": ["T016"],
                            }
                        ],
                        "task_graph": {
                            "nodes": [
                                ...
                                {"id": "T010", "title": "Repair final frontend account component contracts", "status": "completed"},
                                {"id": "T011", "title": "Repair final frontend admin account table components", "status": "completed"},
                                {"id": "T012", "title": "Repair final frontend admin account test modal component", "status": "completed"},
                                {"id": "T013", "title": "Repair final frontend admin account import modal component", "status": "completed"},
                                {"id": "T014", "title": "Repair final frontend admin account reauth modal component", "status": "completed"},
                                {"id": "T015", "title": "Repair final frontend admin scheduled account tests panel", "status": "completed"},
                                {
                                    "id": "T016",
                                    ...
                                    "evidence": [
                                        {
                                            ...
                                            "result": {
                                                ...
                                                "worker_lifecycle": {
                                                    "status": "timed_out",
                                                    "timeout_seconds": 900,
                                                    "timed_out_at": "2026-06-29T00:34:42+00:00",
                                                },
                                            },
                                        }
                                    ],
        ...
        repair_docs = [Path(item) for item in captured[0]["documents"] if "final_verification_repair_resume" in str(item)]
        self.assertEqual(report["status"], "passed")
        self.assertEqual(repair_docs[-1].name, "final_verification_repair_resume_012.md")
        text = repair_docs[-1].read_text(encoding="utf-8")
        self.assertIn("Repair attempt: run_attempt_016", text)
        self.assertIn("Primary failed task IDs: T016", text)
        self.assertIn("Must continue focused task T016: Repair final frontend admin user account components.", text)
        completed_line = next(line for line in text.splitlines() if "Completed tasks to preserve:" in line)
        for task_id in (
            "T001",
            "T002",
